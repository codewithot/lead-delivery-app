...
export async function ensureContactPropertyAssociation(
  ...
  if (!assocId) {
    scopedLogger.error(
      `No association definition found for contact <${CUSTOM_OBJECT_KEY}>`
    );
    return;
  }
...
export function normalizeFreeAndClear(
  ...
  if (value === undefined || value === null) return value as undefined | null; // ← Fixed
...
export function extractGhlId(respData: unknown): string | undefined {
  ...
  return (
    ...
    (get(data, "data", "id") as string | undefined) ||
    (get(data, "record", "id") as string | undefined) ||
    (get(data, "data", "record", "id") as string | undefined) ||
    (get(data, "contact", "id") as string | undefined) ||
    undefined
  );
}
