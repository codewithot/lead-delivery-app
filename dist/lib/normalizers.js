 ...
 */
export function normalizePhone(phone, defaultCountryCode = "US") {
    const warnings = [];
    // Handle null/undefined
    if (!phone) {
        return {
    ...
    if (defaultCountryCode === "US" || defaultCountryCode === "CA") {
        ...
        if (digitsOnly.length === 11 && digitsOnly.startsWith("1")) {
            ...
            warnings.push(`Phone number too short (${digitsOnly.length} digits, expected 10)`);
        }
    ...
    const isValid = defaultCountryCode === "US" || defaultCountryCode === "CA"
...
export function normalizeAddress(address) {
    ...
    if (!isValid) {
        warnings.push("Address validation failed (too short or missing components)");
    }
    return {
        normalized,
        original,
 ...
 * // => "123northmainstreet5"
 */
export function normalizeAddressForFuzzyMatch(address) {
 ...
 */
export function phonesMatch(phone1, phone2, countryCode = "US") {
 ...
 */
export function addressesMatch(address1, address2, useFuzzyMatch = false) {
    if (useFuzzyMatch) {
