 ...
 */
export function dateTimeFromYYYYMMDD(yyyymmdd) {
    const tz = getRegionTimezone();
    const year = parseInt(yyyymmdd.substring(0, 4), 10);
    const month = parseInt(yyyymmdd.substring(4, 6), 10);
    const day = parseInt(yyyymmdd.substring(6, 8), 10);
    return DateTime.fromObject({ year, month, day, hour: 0, minute: 0, second: 0 }, { zone: tz });
}
/**
 * Check if we're within the provision window (06:00 - 06:30)
 */
export function isProvisionWindow() {
    const now = DateTime.now().setZone(getRegionTimezone());
    const hour = now.hour;
 ...
 */
export function formatForLog(dt) {
