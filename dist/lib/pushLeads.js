...
const GHL_ACCOUNTS = [
    ...
    // },
    {
        ...
        locationId: process.env.GHL_LOCATION_IDdirectOneHomeBuyers || "3eqjaHp2WwPxvUWCV9Mb",
        privateToken: process.env.GHL_PRIVATE_TOKENdirectOneHomeBuyers ||
...
export async function pushLeadsForUser(job) {
    ...
    for (const account of GHL_ACCOUNTS) {
        ...
        for (const [contactId, contact] of contactsToPush) {
            ...
            try {
                ...
                if (resp.status === 201 || resp.status === 200) {
                    ...
                    console.info(`✓ [${account.name}] Pushed contact ID ${contact.id} (GHL: ${ghlContactId})`);
                    ...
                    console.error(`✖ [${account.name}] GHL responded ${resp.status} ${resp.statusText} for contact ${contact.id}`);
                }
            }
            catch (err) {
                ...
                if (axios.isAxiosError(err)) {
                    if (err.response) {
                        ...
                        console.error(`❌ [${account.name}] Network error for contact ID ${contact.id}:`, err.message);
                    }
                }
                else {
                    // Not an Axios error
                    const errorMessage = err instanceof Error ? err.message : String(err);
                    console.error(`❌ [${account.name}] Error pushing contact ID ${contact.id}:`, errorMessage);
                }
            }
        }
        accountResults.contacts[account.name] = Object.keys(contactIdMap).length;
        console.info(`✓ [${account.name}] Successfully pushed ${Object.keys(contactIdMap).length} contacts`);
        // ========================================================================
            ...
            try {
                ...
                if (resp.status === 201 || resp.status === 200) {
                    ...
                    if (ghlPropertyId && ghlContactId) {
                        ...
                        console.warn(`⚠️ [${account.name}] Skipping association for property ${p.id}: missing GHL IDs`);
                    }
                }
                else {
                    console.error(`✖ [${account.name}] GHL responded ${resp.status} ${resp.statusText} for property ${p.id}`);
                }
            }
            catch (err) {
                ...
                if (axios.isAxiosError(err)) {
                    if (err.response) {
                        ...
                        console.error(`❌ [${account.name}] Network error for property ID ${p.id}:`, err.message);
                    }
                }
                else {
                    // Not an Axios error (could be any other error)
                    const errorMessage = err instanceof Error ? err.message : String(err);
                    console.error(`❌ [${account.name}] Error pushing property ID ${p.id}:`, errorMessage);
                }
            }
        }
        ...
        console.info(`   👥 Contacts pushed: ${accountResults.contacts[account.name]}`);
