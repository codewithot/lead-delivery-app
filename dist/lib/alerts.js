 ...
 * Send Slack alert
 */
async function sendSlackAlert(job, config) {
    if (!config.slack)
        return;
    const payload = {
 ...
 */
async function sendEmailAlert(job, config) {
 ...
 * Send webhook alert
 */
async function sendWebhookAlert(job, config) {
    if (!config.webhook)
        return;
    const payload = {
        event: "job.failed",
    ...
    try {
        ...
        if (!response.ok) {
            console.error("Failed to send webhook alert:", response.status, await response.text());
        }
        else {
 ...
 */
export async function sendJobFailureAlert(job) {
 ...
 */
export async function sendBatchFailureAlert(jobs, timeRange) {
    const config = getAlertConfig();
    if (config.slack) {
        const payload = {
            channel: config.slack.channel,
            ...
            attachments: [
                {
                    color: "danger",
                    title: `${jobs.length} jobs failed between ${timeRange.start.toISOString()} and ${timeRange.end.toISOString()}`,
                    fields: [
                        {
                            title: "Failed Jobs",
