...
export async function collectMetrics() {
    ...
    const oldestJobAgeSeconds = oldestJob
        ? Math.floor(now.diff(DateTime.fromJSDate(oldestJob.createdAt), "seconds").seconds)
        : 0;
    // Calculate processing rate and estimate completion time
    const processingRate = completedLastMin; // jobs per minute
    const estimatedCompletionMinutes = processingRate > 0 ? Math.ceil(queueDepth / processingRate) : Infinity;
 ...
 */
export function checkAlerts(metrics, config = DEFAULT_ALERT_CONFIG) {
    const alerts = [];
    // Alert 1: Approaching deadline with jobs still pending
    if (metrics.minutesUntilDeadline <= config.deadlineWarningMinutes &&
        metrics.queueDepth > 0) {
        alerts.push(`🚨 DEADLINE WARNING: ${metrics.minutesUntilDeadline} minutes until 7:00 AM, ` +
    ...
    if (metrics.minutesUntilDeadline < 0 && metrics.queueDepth > 0) {
        alerts.push(`🔴 DEADLINE EXCEEDED: ${Math.abs(metrics.minutesUntilDeadline)} minutes past 7:00 AM, ` + `${metrics.queueDepth} jobs still pending`);
    }
    ...
    if (metrics.oldestJobAgeSeconds > config.oldJobThresholdMinutes * 60) {
        ...
        alerts.push(`⚠️ OLD JOB: Oldest job has been pending for ${ageMinutes} minutes ` +
    ...
    if (metrics.queueDepth > 0 &&
        ...
        alerts.push(`⚠️ SLOW PROCESSING: Rate ${metrics.processingRate.toFixed(2)} jobs/min ` + `(minimum: ${config.minProcessingRate} jobs/min)`);
    }
    ...
    if (metrics.queueDepth > 0 &&
        ...
        alerts.push(`⚠️ COMPLETION RISK: Estimated ${metrics.estimatedCompletionMinutes} minutes to complete, ` +
            `but only ${metrics.minutesUntilDeadline} minutes until deadline`);
    }
    // Alert 6: High memory usage
    const memoryThresholdMB = parseInt(process.env.MEMORY_ALERT_THRESHOLD || "600", 10);
    if (metrics.rssMB > memoryThresholdMB) {
        alerts.push(`⚠️ HIGH MEMORY: RSS ${metrics.rssMB} MB ` +
